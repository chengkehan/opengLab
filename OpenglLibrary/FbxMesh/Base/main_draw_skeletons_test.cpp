...
void drawSkeletons()
{
    drawBone(fbxSkeletons.getRootBone());
}

void drawBone(FbxBone* bone)
{
    M3DMatrix44f scale;
    m3dLoadIdentity44(scale);
    m3dScaleMatrix44(scale, 140, 140, 140);
    
    M3DMatrix44f matrix;
    m3dLoadIdentity44(matrix);
    m3dMatrixMultiply44(matrix, scale, bone->getBindpose());
    
    mvTransform.PushMatrix();
    M3DMatrix44f mCamera;
    cameraFrame.GetCameraMatrix(mCamera);
    mvTransform.MultMatrix(mCamera);
    mvTransform.MultMatrix(matrix);
